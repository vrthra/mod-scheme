 ...
 */
static int scheme_load_libs(scheme *sc,const char *init_dir,apr_pool_t *p) {
		apr_status_t rv;
		apr_dir_t *dir;
		apr_finfo_t finfo;
		apr_file_t *file;
...
/* init after DSO load */
static int scheme_post_config(apr_pool_t *pconf, apr_pool_t *plog,apr_pool_t *ptemp, server_rec *s) {
		scheme *interp;
 ...
 */
static sc_config *scheme_create_config(apr_pool_t *p) {
...
static void *create_scheme_server_config(apr_pool_t *p, server_rec *d) {
 ...
 */
static void *create_scheme_dir_config(apr_pool_t *p, char *dir) {
...
static int scheme_read_post_data(request_rec *r , char** buf);
...
static const char* set_init_dir(cmd_parms *parms, void *mconfig, const char *arg) {
		scheme_config_rec *c = ap_get_module_config(parms->server->module_config,&scheme_module);
		c->init_dir =(char*) arg;
		return NULL;
}

static const char* set_max_interps(cmd_parms *parms, void *mconfig,const char *arg) {
		scheme_config_rec *c = ap_get_module_config(parms->server->module_config,&scheme_module);
		c->max_interps = atoi(arg);
		return NULL;
}

static const char* set_min_interps(cmd_parms *parms, void *mconfig, const char *arg) {
		scheme_config_rec *c = ap_get_module_config(parms->server->module_config,&scheme_module);
		c->min_interps = atoi(arg);
		return NULL;
}

static scheme* init_scheme_interp(callback_data *cdata) {
		scheme* interp;
...
static int load_scheme_file(sc_handler *fh, callback_data *cdata,scheme * interp) {
		apr_file_t* file;
...
apr_status_t plain_cleanup (scheme * sc) {
...
static apr_status_t scheme_filter(int type,ap_filter_t *f, apr_bucket_brigade *bb, ap_input_mode_t eMode, apr_read_type_e eBlock, apr_off_t nBytes) {
		int is_input = type;
...
static apr_status_t scheme_output_filter(ap_filter_t *f, apr_bucket_brigade *bb) {
		return scheme_filter(0,f,bb,0,0,0);
}

static apr_status_t scheme_input_filter(ap_filter_t *f, apr_bucket_brigade *bb, ap_input_mode_t mode, apr_read_type_e block, apr_off_t readbytes) {
		return scheme_filter(1,f,bb,mode,block,readbytes);
}

static const char *set_filter(int type,cmd_parms *cmd, void *mconfig,const char *handler, const char *name,ap_filter_type pos) {
		sc_config *conf;
		sc_handler *fh;
...
static ap_filter_type get_pos(const char * position) {
...
static const char *set_input_filter(cmd_parms *cmd, void *mconfig,const char *handler, const char *name,const char *position) {
    return set_filter(1,cmd,mconfig,handler,name,get_pos(position));
}
static const char *set_output_filter(cmd_parms *cmd, void *mconfig,const char *handler, const char *name,const char *position) {
    return set_filter(0,cmd,mconfig,handler,name,get_pos(position));
}

static const command_rec scheme_conf_cmds[] = {
...
static int scheme_read_post_data_1(request_rec *r, char **rbuf) {
